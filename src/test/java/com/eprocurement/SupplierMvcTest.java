...
public class SupplierMvcTest {
	...
	public void testSupplierManagement() throws Exception {
		mvc.perform(get("/supplier/all")).andExpect(status().isOk()).andExpect(view().name("suppliers"));
	}

	//MVC test for get supplier form
	@Test
	public void testGetSupplierForm() throws Exception {
		mvc.perform(get("/supplier/new")).andExpect(status().isOk()).andExpect(view().name("supplier"))
	...
	public void testSaveNewSupplier() throws Exception {
		mvc.perform(post("/supplier/save").with(csrf())).andExpect(status().is3xxRedirection())
	...
	public void testGetSupplierUpdateForm() throws Exception {
		...
		mvc.perform(get("/supplier/1")).andExpect(status().isOk()).andExpect(view().name("supplier"))
				.andExpect(model().attributeExists("supplier"))
